...
public class ReportingServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			DBConnectionUtility DbUtility = new DBConnectionUtility("com.microsoft.sqlserver.jdbc.SQLServerDriver", "jdbc:sqlserver://192.168.1.88:1433;databaseName=haziradb", "sa", "sipl@123");
			...
			request.getSession().setAttribute("Location_Master", Location_Master);
			...
			request.getSession().setAttribute("Lane_Type_Master", Lane_Type_Master);
			...
			request.getSession().setAttribute("Lane_Master", Lane_Master);
			
			PrintWriter JasperReportWriter = new PrintWriter(new String());
			JasperPrint jp = getFilteredReportPrint(null, null, null);
			...
			if(request.getParameter("ExportOption") != null)
			{
				...
				if (ExportOption > 0)
				{
					if(ExportOption == 1)
					{
						JRDesignStyle ContentStyle = new JRDesignStyle();
						ContentStyle.setName("Content_Sans_Serif");
						ContentStyle.setDefault(true);
						ContentStyle.setFontName("DejaVu Sans");
						ContentStyle.setFontSize(10);
						ContentStyle.setPdfFontName("Helvetica");
						ContentStyle.setPdfEncoding("Cp1252");
						ContentStyle.setPdfEmbedded(false);
						jp.addStyle(ContentStyle);
						jp.setPageWidth(600);
						jp.setPageHeight(900);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
				...
				if(request.getParameter("Lane_Type") != null && request.getParameter("Lane_Type") != "")
		...
		request.getRequestDispatcher("/WEB-INF/TransactionReport.jsp").forward(request, response);
	}
	...
	private JasperPrint getFilteredReportPrint(String LocationID, String LaneTypeID, String LaneID) throws Exception {
		...
		{
			if(sb.length() <= 0)
				sb.append(" WHERE ltc.location_id = "+LocationID);
			else
				sb.append(" AND ltc.location_ID = "+LocationID);
		}
		
		if(LaneTypeID != null && LaneTypeID.length() > 0)
		{
			if(sb.length() <= 0)
				sb.append(" WHERE ltc.lane_type_id = "+LaneTypeID);
			else
				sb.append(" AND ltc.lane_type_id = "+LaneTypeID);
		}
		
		if(LaneID != null && LaneID.length() > 0)
		{
			if(sb.length() > 0)
				sb.append(" AND ltc.lane_id="+LaneID);
			else
				sb.append(" WHERE ltc.lane_id="+LaneID);
		}
		
		ReportingUtility ru = new ReportingUtility();
		DynamicReport dr = ru.buildReport(sb.length() > 0 ? sb.toString() : null);
		DBConnectionUtility DBUtility = new DBConnectionUtility("com.microsoft.sqlserver.jdbc.SQLServerDriver", "jdbc:sqlserver://192.168.1.88:1433;databaseName=haziradb", "sa", "sipl@123");
